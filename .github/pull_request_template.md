<!-- 全体のワークフロー・チェックリストの位置づけは CONTRIBUTING.md を参照 -->
...
**batch size / review 分離 / adversarial checks（CONTRIBUTING.md §9）**
- [ ] batch size は tier 別基準（`full`: 15–25、`gloss`: 25–40）の範囲内、または
      超過理由と review 単位を本 PR 本文に明記した
- [ ] public theorem / `sorry`・scaffold / パラメータ付き主張 / named theorem 引用が
      含まれる場合、author とは別の reviewer が exact Lean type と source を直接確認した
- [ ] 該当する adversarial checks（§9 の 6 項目: 量化子・仮定・結論の一致／端点・退化
      パラメータ／暗黙の仮定／named theorem の一次資料一致／special case と generic の
      混同／典型的反例）を確認した
