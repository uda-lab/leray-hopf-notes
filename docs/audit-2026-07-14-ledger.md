...
- **維持**: 複数の数学的構成要素（空間・埋め込み・作用素・汎関数・形式など）を一つの抽象的なまとまりとして述べる用法。
  該当は `DissipativeEvolution` / `torus3Evolution` の「散逸発展の束」（空間 $H$ ＋正則性汎関数＋粘性形式＋移流形式＋テストクラス）、
  GLOSSARY の「近似射影族の束」（近似射影の族）、「主線の束」（Gelfand 三つ組＝空間 $V$, $H$ ＋埋め込み $\iota$ とその性質）。
  GLOSSARY に見出し語を追加して定義した（定義は同見出し語と同一。作用素・汎関数に限らないことに注意 — 射影族も空間・埋め込みも含む）。
...
- **lint**: `docs/GLOSSARY.md` の `forbidden` 欄に、裁定で不適切とされた複合語のみ（全 7 語: `形式束` / `解の束` / `解束` / `データ束` / `データの束` / `弱解の束` / `コンパクト性の束`）を登録し、既存の `glossary_lint.py` に強制させた。助詞を挟む形（`領域データの束` `形式データの束` `解データの束` `時間軟化データの束`）は `データの束` が覆う。単独の `束` および一般形の `の束` は**禁止していない** — 維持用法 3 つがいずれも「〜の束」の形のため。したがって `収束` / `拘束` / `束縛` と維持用法は誤検出しない。発火すること・誤検出しないことの双方を機械確認済み。
  なお本行の語数・語彙は `docs/GLOSSARY.md` の当該行が正典であり、齟齬が生じた場合はそちらを参照すること。
