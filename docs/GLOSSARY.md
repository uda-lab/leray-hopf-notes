...
| viscous form | 粘性形式 | `DissipativeEvolution.viscousForm`, `stokesTestPairing`, `stokesTestPairing_R3` | 非スケールの双線形形式 `a(u,v)=∫∇u:∇v`（具体的な Fourier 実現は「Stokes pairing」行を参照）。`ν` はこの形式自体の定義には含まれず、弱形式 `WeakFormNS` や粘性散逸形式 `viscousFormSq` など消費側が外側から掛ける（issue #67） | 「`ν∫∇u:∇v` を粘性形式自体の定義とする言い換え」 |
