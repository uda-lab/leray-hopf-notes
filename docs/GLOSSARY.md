...
| witness | （文脈に応じて直接記述） | | 数学的な witness（存在を裏付ける具体的対象・値）を名詞として訳語化しない。文脈に応じて直接記述する：「非自明性の witness 定理」→「$b\neq0$ を直接示す定理」のように述べる対象を明示するか、「witness を与える」→「条件を満たす具体的な対象を与える」、または「$b(u,v,w)\neq0$ となる具体的な $u,v,w$ の構成」のように構成そのものを書き下す（issue #105 の owner 裁定）。同じ理由で raw English "witness" を和文へ直接融合させた語（例：「witness 一致計算則」「component-witness 形」）も禁止 — 訳語化ではなく Lean 宣言名（`[[display|LerayHopf.foo_witness]]` の `target` 側）としての出現のみ許容する | 「証人」、「目撃者」、raw "witness" を和文へ融合させた表現（`scripts/glossary_lint.py` の witness-fusion チェックが検出） |
