...
| band-limited | 帯域制限（された場） | `viscousEnn_eq_ofReal_of_bandlimited` | ある次数の Fourier 截断で固定される（Fourier 台が有限の）場 | 「バンド制限」 |
...
| lane | 系列 | `exists_lerayHopf_r3` / `exists_lerayHopf_torus3` | ℝ³／トーラス両証明系列を指すリポジトリ内部語。和文散文では「ℝ³ 系列」「トーラス系列」と書く | 「レーン」 |
...
| equi-Lipschitz | 同程度 Lipschitz | `Bochner.WeakLimitToolkit.cauchySeq_of_equiLipschitz_of_dense` | 同程度連続（equicontinuous）に倣う訳。共通の Lipschitz 定数をもつこと | 「等 Lipschitz」 |
